<template>
    <div class="article-list-box" v-for="item in state.list" :key="item.id">
        <p class="name">
    ...
    <div class="pagation-box" v-if="state.list.length">
        <ul>
            <li class="prev" @click="JumpPage('prev')"><img src="../../assets/prev.png" alt="" srcset="" /></li>
            <li :class="[state.searchForm.currentPage == item ? 'current' : '']" @click="JumpPage('page', item)" v-for="item in state.pagation.pageNum" :key="item">{{ item }}</li>
...
<script lang="ts" setup>
    ...
    const GetList = async () => {
        ...
        if (data.code) {
            ...
            console.log(state.pagation.pageNum)
        }
    }
