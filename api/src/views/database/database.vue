<template>
    <div class="database-box">
        ...
        >
            <el-form>
                <el-form-item label="字典名称：" label-width="120">
...
import { addTag, findTag, delTag, updateTag } from '@/api/tag'
@Component({
    name: 'Database'
})
export default class Database extends Vue {
    ...
    // 新增 或 修改
    public async AddOrUpdateTag() {
        if (this.type === 'add') {
            const { data } = await addTag(this.formData)
            if (data.code) {
            ...
            const { data } = await updateTag(this.formData)
            if (data.code) {
    ...
    public async Del(val: {id: string}) {
        ...
        this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
            ...
            const { data } = await delTag({ id: val.id })
            if (data.code) {
    ...
    public async Search() {
        const { data } = await findTag<any>(this.searchForm)
        if (data.code) {
